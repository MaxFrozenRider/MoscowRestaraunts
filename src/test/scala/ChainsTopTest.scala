...
class ChainsTopTest extends AnyFlatSpec  with SparkSessionTestWrapper
with BeforeAndAfter with DataFrameComparer {
  ...
  before {
    testDf = spark.read.option("header", true).csv("src/test/resources/chains_top/moscow_places_clean.csv")
  }
